...
def run_coding_task(task_description: str, category: str | None = None) -> dict:
    ...
    qwen3_enabled = is_framework_integration_enabled("qwen3_coder_next_coding_agent")
    client = None
    if not runpod_enabled and not devstral_enabled and not qwen3_enabled:
        # None of the three RunPod backends has an Anthropic SDK client and
        # none needs ANTHROPIC_API_KEY at all -- only construct/require it
        # for the Claude-backed paths (inline loop or LangGraph).
    ...
    requirements = (
        extract_task_requirements(task_description) if (runpod_enabled or devstral_enabled or qwen3_enabled) else None
    )

    if runpod_enabled or devstral_enabled or qwen3_enabled:
        ...
        # are untouched. run_via_devstral()/run_via_qwen3() actually build
        # their own system prompt internally (native tool-calling needs no
        # prompted <tools> format instructions -- see
        # build_devstral_system_prompt()'s own comment), so this value is
        # discarded on those paths; computed the same cheap way regardless
        # rather than adding extra branches here.
    ...
    if runpod_enabled:
        ...
        # Lazy import, same rationale as the runpod/devstral branches above
        # -- this module (and nova_remote_inference_qwen3_native_tools) is
        # only ever imported when this flag is on.
        from nova_orchestrator_qwen3 import CODING_AGENT_MAX_OUTPUT_TOKENS, run_via_qwen3

        final_status, turns_used = run_via_qwen3(
            system_prompt,
            messages,
            root,
            slug,
