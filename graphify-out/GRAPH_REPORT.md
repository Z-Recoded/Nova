...
Cohesion: 0.08
Nodes (24): browser_hands/config/sites.yaml — per-adapter config template, 10. Change Log, 11. Session Startup Checklist, 1. Project Overview, 4. Python Style Guide, 5. Known Issues & Active Bugs, 7. Nova API Routes, 8. What Claude Should Always Do (+16 more)

### Community 5 - "Headless Dispatch & Escalation"
Cohesion: 0.06
Nodes (38): ActivityProfilePushRequest, cancel_escalation(), create_tool_approval(), get_activity_profile(), get_dispatch_pause_route(), get_escalations(), get_tier_proposals(), get_tier_watermarks() (+30 more)
...
Cohesion: 0.06
Nodes (54): _execute_tool_guarded(), Wraps nova_orchestrator._execute_tool() with two extra checks beyond     nova_o, Run one task through qwen3:8b via Ollama's tool-calling API, dispatching     ev, run_spike_task(), _worktree_has_file(), ClickUp 86barguac — Nova Skills Library, Runs one task through the RunPod backend directly (bypassing     run_coding_tas, run_runpod_backend() (+46 more)
...
Cohesion: 0.06
Nodes (38): agent_task(), AgentTaskRequest, ask_nova(), AskRequest, create_escalation(), create_tier_proposal(), DispatchPauseRequest, EscalationCreateRequest (+30 more)
...
Cohesion: 0.07
Nodes (40): get_in_flight_status(), Is a headless dispatch running right now, and if so which task —     backs the, get_max_unreviewed_dispatches(), is_review_backpressure_enabled(), is_sandboxed_dispatch_enabled(), True if nova_scheduled_dispatch.py's review-bandwidth cap is on     (86bawpvzz, True if nova_scheduled_dispatch.py's cron loop should run each dispatch     thr, The review-backpressure cap threshold — nova_scheduled_dispatch.py     skips pi (+32 more)
...
Cohesion: 0.20
Nodes (15): InputStream, _ensure_models_downloaded(), _load_stt_pipeline(), ndarray, Root-mean-square amplitude of an int16 audio chunk -- simple speech/silence sign, Accumulate audio chunks from an already-open stream until the speaker     goes, Run the STT pipeline on a raw int16 audio array, bypassing ffmpeg entirely, Synthesize text to audio and play it back through the default output device. (+7 more)
...
Cohesion: 0.07
Nodes (45): nova_config.json (feature flag values), FlagMeta, get_approval_gate_max_files_per_turn(), get_approval_gate_patterns(), get_approval_gate_poll_interval_seconds(), get_approval_gate_timeout_seconds(), get_flag_registry_values(), is_augment_enabled() (+37 more)
...
Cohesion: 0.04
Nodes (87): BackendProfile, One backend's real conventions: how it's asked to call tools, how a     finishe, is_framework_integration_enabled(), True if the named framework integration flag is on. These are     independent f, _ensure_initialized(), log_gate_result(), log_guard_events(), log_turn() (+79 more)
...
Cohesion: 0.04
Nodes (80): AST, ExceptHandler, expr, _added_lines_text(), _assignment_target_names(), _base_commit_ruff_violations(), _check_cross_module_missing_exports(), _check_deliverables_present() (+72 more)
...
### Community 155 - "example.py"
Cohesion: 0.25
Nodes (3): Counter, four_of_a_kind(), full_house()
