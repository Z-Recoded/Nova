...
├── nova_diff_link.py       # Trace-to-diff linking (86bb7pb6t) — given a branch, resolves a GitHub compare link (pushed) / local diff (same-machine ref) / "unavailable", backing GET /observability/diff. Same-machine-only v1, no cross-machine SSH leg
...
| /observability/diff | GET | ✓ Working | Diff/PR-link lookup for one branch (86bb7pb6t) — GitHub compare link if pushed to origin, local `git diff master...<branch>` if the ref exists on this machine, "unavailable" otherwise (pruned, or local-only on the other machine — same-machine-only v1). Lazy/on-demand, not part of /observability/uncertainty's payload. Not token-gated (read-only) |
