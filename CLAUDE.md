...
| /qwen-swap-status | GET | ✓ Working | Progress toward Phase 3.5's Qwen3 8B swap trigger — full Aero+Omen combined count when served from the Aero, Omen-only when served from the Omen (86bb3cey2) |
| /worktree-status | GET | ✓ Working | Open git worktrees on both machines with age/merged/prunable status — same Aero-vs-Omen `view` pattern as `/qwen-swap-status` (86bb3ceyc) |
