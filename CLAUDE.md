...
- `nova_scheduled_dispatch.py` — the real cron entry point for
  ...
  **Known, accepted gap:** one task per 2-hour firing bounds the *rate*
  ...
  (3 unreviewed → 2 after recording one review). **Known gap, not yet done:**
  the two real existing dispatches (`86bayjdrh`, `86baux7bb`) predate this
  feature and have no review-log entry yet — worth a retroactive
  `--review` call for each so the count starts accurate rather than
  carrying two permanent phantom "unreviewed" entries once the flag is
  turned on
