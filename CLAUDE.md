...
| 2026-07-19 | Shipped `nova_remote_inference.py` — RunPod serverless adapter (`86baw3010`), gated behind `remote_gpu_inference` in `nova_config.json`/`.py` (default off), wired into `nova_query.py`'s `ask()` | Closes the "workhorse" short-term goal — real endpoint (Qwen2.5-Coder-32B-Instruct-AWQ) already deployed, just needed the adapter. Real schema found live differs from the ticket's OpenAI-style assumption (RunPod's own raw vLLM-worker format instead). Real bug found and fixed during live verification: `query_log.jsonl` logged the locally-routed model name even when the remote call actually generated the response — fixed by tracking the real model used separately. Verified live both directions: flag on (real RunPod generation, confirmed invoked exactly once) and flag off (confirmed zero behavior change) |
| 2026-07-19 | Shipped `nova_agentic_dataset_curator.py` — external-dataset half of `86bara7zk` (agentic reasoning training data), 4-phase CLI (`--download`/`--convert`/`--blend`/`--report`), added `pyarrow` to `requirements.txt` | Real dataset identifiers/sizes/licenses verified live against Hugging Face Hub rather than trusting the ticket's claims, which had real inaccuracies (AgentInstruct is 1,866 examples not ~25K, license unconfirmed — used anyway per Marvin's call; ToolBench's official HF source 401s, used a verified Apache-2.0 mirror; TAT-QA is CC-BY-4.0 not MIT/Apache — included anyway). Real schema surprises caught by inspecting live downloaded rows before writing parsers (ToolBench's `conversations` field is parallel arrays, not a list of dicts; FinQA's `question`/`answer` live nested under `qa`, and its HF repo has no data files at all — just a loader script pointing at a GitHub zip, downloaded directly instead). Ran end-to-end: real curated 10,000-row `.jsonl` at `data/agentic_training/curated/`, hit the 60/40 agentic/financial target exactly. Nova-augmentation slice and the actual fine-tune run stay deferred (blocked on MCP tool-calling, `86baf72n5`) |
| 2026-07-19 | Shipped `nova_voice.py` — Minimal-tier voice input/output (`86baeyg3q`, scoped down: local wake word + local STT + local TTS, desk-only, not the full ticket's phone/watch capture); added `openwakeword`/`piper-tts`/`sounddevice`/`torchaudio` to `requirements.txt`; fixed `CHROMA_HOST` in `nova_query.py`/`graph_builder.py`/`ingest.py` to the Omen's Tailscale IP instead of its LAN IP (Section 2's Key External Dependencies updated) | Real technology research picked `openwakeword` (Porcupine's free tier doesn't cover custom wake words), `distil-whisper` via already-installed `transformers` (avoids `faster-whisper`'s `ctranslate2` dependency chain, which has a documented crash history on RTX 50-series cards), and `piper-tts`'s real Python API. Every pipeline component verified individually (wake-word scoring, a full Piper→Whisper round trip, real mic stream open/read) but the live wake-word-trigger-on-real-speech loop was never run by the agent (can't physically speak) — genuinely needs Marvin. While attempting that live test, hit a real, reproducible bug: `nova_query.py`'s hardcoded Chroma LAN IP (`192.168.1.250`) fails whenever the Aero isn't on the home network, even though the Omen itself was confirmed up and reachable the whole time over Tailscale — fixed permanently by switching to the Tailscale IP (`100.114.197.117`), confirmed working from both home and away. Also found: this venv's `torch` is CPU-only (no CUDA) despite the RTX 5070 being available — STT runs slow (~8s for a 2.5s clip); left as a flagged, deliberate non-fix pending Marvin's call on a ~2-3GB CUDA torch reinstall. Wake word itself is a `hey_jarvis` placeholder (openWakeWord has no pretrained "Hey Nova" — real custom training is a separate ~1hr follow-up) |
...
| 2026-07-21 | Fixed hardcoded `C:/Nova` paths in `nova_logger.py` (`LOGS_DIR`) and `nova_corrector.py` (`JSONL_PATH`, dotenv path) — the real cause of the training-data widget showing 0/100 on the Omen; filed `86bb1pkpb` for the 15 other remaining instances found by the same grep sweep | Same bug class already fixed 8 times before (`GRAPH_PATH`, `CONFIG_PATH`, `CHROMA_HOST`, etc.), just never caught in these two files until this feature made the Omen-side breakage visible. Confirmed via SSH the Omen has no `training_flags.jsonl` at all — `log_blend()` has likely never fired for real there, not silently corrupted data |
| 2026-07-21 | Fixed the same hardcoded-path bug in `nova_log.py` (`LOGS_DIR`) and `nova_benchmark.py` (`LOGS_DIR`/`BENCHMARK_LOG_PATH`), picked off `86bb1pkpb`'s list at Marvin's request | Confirmed real, user-facing impact as suspected: the Nova Log Health/Benchmark dashboards had this identical "0 data on the Omen" bug. Fixed both together deliberately — `nova_log.py`'s `get_benchmark_runs()` imports `BENCHMARK_LOG_PATH` from `nova_benchmark.py`, so fixing only `nova_log.py`'s own `LOGS_DIR` would have left the Benchmark view still broken. Verified live on the Aero: real health summary (158 queries, 12% blend rate) and 3 real benchmark runs both read back correctly through the fixed paths |
