...
- Phase 1.5  | Self-Monitoring        | Resource headroom calculator ✓ v1 live (2026-07-05 — `nova_headroom.py`, `GET /headroom`); Task Scheduler auto-start for nova_watcher.py + nova_api.py/Open WebUI still backlog; periodic benchmarking suite still backlog — doubles as the eval harness for Phase 3/3.5's model-swap criteria below
...
- Phase 3    | First Fine-Tune        | Backlog — Unsloth + DPO → GGUF → Ollama (conversational/lore lane); base-model re-eval (Llama 3.2 3B vs. Phi-4 Mini 128K) + dynamic model routing. **Swap trigger:** run Phase 1.5's benchmarking suite head-to-head on Nova's own existing metrics (`blend_rate_last_100`, `avg_latency_ms_24h` from `nova_log.py`) — swap only when a candidate clearly wins, not on a fixed timeline
- Phase 3.5  | Coding Agent Lane      | ✓ v1 live (2026-07-05) — Claude API-backed coding sub-agent (`nova_orchestrator.py`), git-worktree isolated, no Docker/OpenHands yet (deferred as a hardening pass); proven on one real merged task (`86baeyg8b`, commit `c516cca`). **Qwen3 8B swap trigger, not a fixed date:** (1) ~30-50 diverse real task transcripts accumulated in `logs/agent_log.jsonl` — already happening automatically every real `/agent/task` run, not a separate curation project; (2) ~20% held out as a never-trained-on eval set (same benchmarking suite as Phase 1.5/3); (3) swap only once Qwen3 clears a defined pass bar against that held-out set (completion rate within turn budget, no worse than Claude's baseline on the same tasks) — this is the path to Nova coding independently
- Phase 4    | Roaming Layer          | **Lightweight v1 recommended next:** Tailscale (or similar mesh VPN) on the current machine + Task Scheduler auto-start/no-sleep gets "Nova reachable from any device, anywhere" with no new hardware and ~an hour of setup. Heavier items (headless Ubuntu server, Dockerized services, cloud GPU, hosted inference fallback) deferred until there's a concrete forcing function (freeing the daily-driver machine, long unattended fine-tune runs) — not a prerequisite for "always present"
