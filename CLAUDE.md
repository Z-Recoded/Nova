...
├── nova_orchestrator_devstral.py # Devstral (native tool-calling) backend for the turn loop — 2nd real coding-agent model, alongside the RunPod/Qwen prompted-format backend (devstral_coding_agent flag, default off)
...
├── nova_tool_calling_execution.py # Tool-calling track's Phase 1/2 (86bbcfpbg) — 15 hand-authored tasks against 3 of nova_mcp_server.py's real tools, graded via real execution against a running nova_api.py
├── nova_gpu_monitor.py     # Background GPU telemetry logger for the Aero's RTX 5070 (temp/power/clocks/real throttle-reason flags via nvidia-smi) — logs/gpu_telemetry_log.jsonl, --summary mode, optional live terminal view
├── nova_guard_stats.py     # Guard/completion-gate firing attribution CLI — tallies guard_events_log.jsonl/ground_truth_gate_log.jsonl by guard id/check name (86bb4gy0y)
├── nova_langfuse_client.py # Langfuse Cloud tracing for the coding sub-agent (Observability Initiative 86bb7pamh) — log_turn()/log_guard_events()/log_gate_result(), langfuse_tracing flag, default off
├── nova_laminar_client.py  # Laminar Cloud tracing (sibling infra ticket 86bb7qudh) — mirrors nova_langfuse_client.py's log_turn()/log_guard_events()/log_gate_result() calling convention, fires additively alongside it from the same 3 turn-loop call sites; session-grouping + span-metadata instead of Langfuse's trace-id/score shapes (Laminar's own idiom), laminar_tracing flag, default off
├── nova_observability_dashboard.py # Data layer for the /observability trend dashboard (86bb7pb20) — failure frequency over time, per-model comparison, uncertainty-vs-outcome
├── nova_observability_dashboard.html # /observability dashboard page — hand-rolled inline SVG, no charting library (86bb7pb20)
├── nova_diff_link.py       # Trace-to-diff linking (86bb7pb6t) — given a branch, resolves a GitHub compare link (pushed) / local diff (same-machine ref) / cross-machine diff (via the Omen-Aero SSH bridge's 6th key, AERO_DIFF_KEY) / "unavailable", backing GET /observability/diff
...
├── nova_mlflow_ingest.py   # Reads a real fine-tune run's mlflow_run_metadata.json (written by the two finetune scripts, uploaded to HF Hub alongside the checkpoint since the rented pod has no Tailscale access) and logs it into the Omen's MLflow server — run from the Aero, never the pod (86bb7quga)
...
├── nova_observability_status.py # Cross-machine guard/gate/agent_log data backing /observability/per-model + /observability/failure-frequency — same combined/omen_only/aero_only pattern, 7th Omen-Aero SSH key (86bb7pb20 follow-up, 2026-08-06)
...
├── docker/
│   ├── nova-aci-sandbox/   # Dockerfile for nova_aci_harness.py's sandboxed execution — built + functionally verified on the Omen only (Docker isn't on the Aero)
│   └── nova-dispatch-sandbox/ # Docker sandboxing for headless coding-agent dispatch (86baf72qq/86barex1u, 2026-07-23) — used by nova_omen_dispatch.dispatch_headless_task_sandboxed()
...
| /observability | GET | ✓ Working | Observability trend dashboard page (HTML) — failure frequency over time, per-model comparison, uncertainty vs. outcome (86bb7pb20) |
| /observability/failure-frequency | GET | ✓ Working | Real A1-G2 registry-code fires by date, from local JSONL (guard_events_log.jsonl + ground_truth_gate_log.jsonl) |
| /observability/per-model | GET | ✓ Working | Per-model run count, gate pass rate, registry-code fires, outcome-bucket counts — local JSONL joined to agent_log.jsonl's model field via branch |
| /observability/uncertainty | GET | ✓ Working | Per-token logprob uncertainty vs. locally-computed outcome bucket, from Langfuse Cloud (not local JSONL — logprobs are never persisted locally). Reads regardless of the langfuse_tracing write-flag; fails open (available: false) if credentials aren't configured |
| /observability/diff | GET | ✓ Working | Diff/PR-link lookup for one branch (86bb7pb6t) — GitHub compare link if pushed to origin, local `git diff master...<branch>` if the ref exists on this machine, a diff fetched from the OTHER machine over the Omen-Aero SSH bridge if it only exists there, "unavailable" if pruned everywhere. Lazy/on-demand, not part of /observability/uncertainty's payload. Not token-gated (read-only) |
...
- **Dual-remote push (added 2026-08-14):** every push now goes to both `forgejo` (self-hosted,
  `marvinbell/Nova` on the Omen's Forgejo instance, `86bb7quk1`) and `origin` (GitHub,
  `Z-Recoded/Nova`) — Marvin's stated preference for the self-hosted environment, so `forgejo`
  is pushed first. Remote config: `git remote add forgejo nova-forgejo:marvinbell/Nova.git`, an
  `~/.ssh/config` alias (`nova-forgejo` → the Omen's **Tailscale IP** `100.114.197.117`, port
  `222`, `IdentityFile ~/.ssh/id_ed25519_forgejo_nova`) — Tailscale IP deliberately chosen over
  the LAN IP already-established sibling entry (`vivarium-forgejo`, a different, unrelated
  project) uses, applying the same lesson Section 2's Chroma LAN-IP-fragility fix already
  learned: a LAN IP only resolves from the home subnet, Tailscale works from anywhere. The
  `id_ed25519_forgejo_nova` deploy key (write access, scoped to just this one repo — same
  least-privilege pattern as the Omen's own read-only GitHub deploy key) was found already
  sitting unused in `~/.ssh/` from an earlier, never-finished session — reused rather than
  regenerated. `nova_omen_sync.py` is untouched by this — it still only pulls from `origin`
  into the Omen's main checkout; the Forgejo push is a second copy of history, not a new
