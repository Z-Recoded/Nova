# nova_orchestrator_qwen3.py
# Qwen3-Coder-Next-80B-A3B backend for nova_orchestrator.py's coding
# sub-agent turn loop -- a third RunPod-hosted alternative to the
# Claude-API-backed inline loop, alongside nova_orchestrator_runpod.py's
# Qwen2.5-Coder-32B backend and nova_orchestrator_devstral.py's Devstral
# backend.
#
# Only imported by nova_orchestrator.py when framework_integrations.
# qwen3_coder_next_coding_agent is enabled in nova_config.json -- importing
# this module never happens when the flag is off (the default).
#
# Unlike nova_orchestrator_runpod.py's prompted <tools>-tag format (needed
# because Qwen2.5-Coder has no reliable vLLM tool-call parser), this
# backend uses REAL native tool-calling: OpenAI-style `tools`/`tool_choice`
# request params and a `tool_calls` array in the response, via
# nova_remote_inference_qwen3_native_tools.chat_with_tools() and vLLM's own
# `qwen3_coder` tool-call parser. Confirmed live 2026-08-06 against the
# deployed endpoint (Qwen/Qwen3-Coder-Next-FP8) that this produces a
# correctly populated tool_calls array, not just a 200 status.
#
# Structurally a copy of nova_orchestrator_devstral.py -- same native
# message-history shape (role="tool" messages carrying a tool_call_id, an
# assistant message that called tools has content=None plus a separate
# tool_calls field), same reason for being a separate module rather than a
# second mode bolted onto nova_orchestrator_runpod.py: every guard function
# below that doesn't care about message shape is imported and reused
# unchanged; only the turn loop, system prompt, and context-window pruning
...
import nova_remote_inference_qwen3_native_tools as native_inference
from nova_backend_profiles import QWEN3_CODER_NEXT_PROFILE
...
def _log_agent_turn_qwen3(
    ...
    """
    Sibling to nova_orchestrator_devstral._log_agent_turn_devstral(),
    writing the identical agent_log.jsonl schema from this backend's real
    native response shape. No near_miss concept here (unlike the
    prompted-format backend) -- native tool-calling is parsed server-side
    by vLLM, so a malformed tool-call text burst structurally cannot
    happen; stop_reason is only ever "tool_use" or "end_turn".
    """
    ...
    entry = {
        ...
        "backend_profile": QWEN3_CODER_NEXT_PROFILE.name,
...
def run_via_qwen3(
    ...
    """
    Runs the turn loop via Nova's RunPod-hosted Qwen3-Coder-Next-80B-A3B
    endpoint, using real native tool-calling. Same (final_status,
    turns_used) contract as run_via_runpod()/run_via_devstral()/
    run_via_langgraph(). `system_prompt` is accepted for interface parity
    with the other backends' call signature but ignored -- this backend
    builds its own system prompt via build_qwen3_system_prompt(), matching
    how the other two RunPod backends also build their own rather than
    ...
    messages = [
        {"role": "system", "content": build_qwen3_system_prompt()},
        *messages,
    ]
    ...
    for turn in range(1, max_turns + 1):
        ...
        if response is None:
            final_status = "stopped_qwen3_call_failed"
            break
